...
## 2026-08-07 cross-sport calibration update

- Re-fit `data/calibration/calibration_curves_latest.json` — **TENNIS now included** (n≈18.9k, ECE≈0.123, `inverted_top`).
- Graded scalar refresh applied for **MLB + WNBA only**. Tennis Goblin still **not** raised toward the 0.50 policy target (would inflate).
- Root cause of Tennis STRONG domination: `_resolve_leg_prob` preferred L5/line HR (often 0.80–0.85) over calibrated `ml_prob` (~0.58 on today's STRONG tennis legs). **Fix:** Tennis skips the L5 early-return and uses capped `ml_prob` first.
- Do **not** compare sports on raw `edge` (Tennis game units ≫ MLB/WNBA). Prefer `prop_quality_score` / calibrated `ml_prob` / `blended_score`.
- Audit script: `scripts/audit_strong_leg_calibration.py` → `data/reports/strong_leg_calibration_latest.json`.
...
| `PROPORACLE_MAIN_MAX_LEGS` | **3** | Mixed MAIN short book stays 2–3 |
| `PROPORACLE_GOBLIN_MAX_LEGS` | **6** | Goblin-only / Long Goblin may emit 4–6 |
| `PROPORACLE_LONG_PARLAY` | **1** (on) | 5–6 long-parlay sidecar enabled |
