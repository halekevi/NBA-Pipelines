#!/usr/bin/env python3
"""
Flag NBA1Q/NBA1H/WNBA1Q/WNBA1H graded_props rows likely graded with full-game stats.
...
def process_file(path: Path, *, dry_run: bool) -> tuple[int, int]:
    ...
    for p in props:
        ...
        if sport not in ("NBA1Q", "NBA1H", "WNBA1Q", "WNBA1H"):
            continue
        try:
...
def main() -> int:
    ...
    print(f"\n{mode} {total_flagged} NBA/WNBA period rows across {len(paths)} files")
    return 0
