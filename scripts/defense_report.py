...
Overall rank is possession-based DEF_EFF from box scores when available
(opp points / opp possessions). ESPN opp PPG is kept as a secondary column.
...
def main():
    ...
    if "OPP_PPG" in df.columns and df["OPP_PPG"].notna().sum() >= 3:
        ...
        df["OPP_PTS_RANK"] = rank_series(df["OPP_PPG"], ascending=True)
