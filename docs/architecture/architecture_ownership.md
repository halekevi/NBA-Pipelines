...
|---|---|---|---|---|---|---|
| 1) Live tickets JSON | `tickets_latest.json` | `ui_runner/templates/` (runtime), `ui_runner/data/` (snapshot copy), `mobile/www/` (bundle copy); **live site reads GitHub origin/main** | Mixer from `scripts/combined_slate_tickets.py --write-web`; dual card from `scripts/build_goblin70_tickets.py --write-web` (Goblin-70 first, graded-main mixer under); `scripts/Publish-LiveSite.ps1` pushes to origin/main; mobile copy by `scripts/generate_mobile_bundle.py` | `ui_runner/app.py` `/tickets` (renders prebuilt JSON; does not rebuild slips) | Keep latest in runtime path; archive dated history in `ui_runner/data/`; mobile keeps deploy copy | **High** (local checkout is not what Railway serves) |
| 1b) Live slate / winrate JSON | `tickets_winrate_latest.json`, `winrate_goblin_opt3_shadow_latest.json`, `slate_latest.json`, `slate_sport_*.json`, `sport_breakdown.json` | `ui_runner/templates/` (runtime), `ui_runner/data/` (snapshot copy), `mobile/www/` (bundle copy) | `scripts/combined_slate_tickets.py` via `--write-web`; orchestrated by `run_pipeline.ps1`; mobile copy by `scripts/generate_mobile_bundle.py` | Slate APIs and High Leg HR panel on `/tickets` | Keep latest in runtime path; archive dated history in `ui_runner/data/` | **High** (same logical "latest" exists in multiple locations) |
...
- Live Railway `/tickets` should read **origin/main**, not an unpushed local checkout.
