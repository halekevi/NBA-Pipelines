...
| 12) Pipeline status | `pipeline_status.json` (and associated slate status markers, e.g. `slate_display_date.json`) | **Disk:** `ui_runner/runtime/`. **GitHub raw:** `ui_runner/templates/`. `mobile/www/` is bundled fallback only. | `run_pipeline.ps1` status writers + mobile bundle step | UI freshness badges/cards | Latest-only runtime + templates mirror; dated snapshots under `outputs/<date>/` | **High** (local checkout is not what Railway serves) |
