"""Ticket-eval track + group allow for STRONG Standard HOT / Mix shadows."""
...
def test_normalize_eval_track_strong_mix_shadow():
    assert _normalize_eval_track("strong_mix_shadow") == "strong_mix_shadow"
    assert _normalize_eval_track("strong-mix") == "strong_mix_shadow"
...
def test_group_is_strong_mix_names():
    assert _group_is_strong_mix("STRONG Mix")
    assert _group_is_strong_mix("STRONG Mix 3-Leg")
    assert not _group_is_strong_mix("STRONG 3-Leg")
    assert not _group_is_strong_mix("STRONG Standard HOT")
...
def test_group_allowed_for_strong_mix():
    assert _group_is_allowed("STRONG Mix", pool_mode="strong_mix_shadow")
    assert _group_is_allowed("STRONG Mix 2-Leg", pool_mode="")
